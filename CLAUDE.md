...
**Editor interaction model** ([src/editor/](src/editor/)). Two distinct gestures on the mosaic: *hover* inspects, *drag* edits. Hovering a minute frames the whole contiguous activity run (grouped by `source_key`, or by category for source-less manual edits) via an edge-cancellation outline in [mosaic.ts](src/mosaic/mosaic.ts) `drawActivityFrame`, and shows a tooltip of app/span/length/category — the per-minute `source_title` already rides along in `MinuteCell`, this only surfaces it. **A single-minute click classifies the whole app/source** (`reclassify_source` — recolors its past + sets a future override; green-accented popover with `SOURCE_CHOICES`). **A drag across minutes is a manual locked per-minute edit** (`setSegment`/`clearSegment`; amber-accented popover with `MANUAL_CHOICES`). Click on a source-less minute (void/idle) falls back to manual edit. The two popovers are visually distinct (accent + header + scope subtitle). The open popover dismisses cleanly because its outside-pointerdown handler calls `stopPropagation()` in the capture phase, so the click never reaches the canvas to start a new selection (a plain flag fails — microtasks drain between listeners). Hover is suppressed while dragging or while the popover is open.

**Renderer sizes itself against `canvas.parentElement`, not the document.** The window is `flex: column`: a 24 px **top bar** (`#status-bar`, see [index.html](index.html) + [main.css](src/main.css)) then the mosaic-host (`flex: 1`, gets remaining height). The top bar is the window drag handle (`-webkit-app-region: drag`; `decorations: false`), and holds the hamburger menu, the ticker, the copy-status button, and custom window controls (minimize / maximize / close→`quit_app`) — all `app-region: no-drag`. There is no separate invisible drag-region anymore. If you add chrome, keep canvas inside its own flex child or the renderer paints under it.
...
- **Read-only-ish main UI.** Chrome is confined to the thin top bar (hamburger menu + window controls); the mosaic body itself stays control-free. Everything else lives in popovers (hour editor) or separate windows (history, settings).
...
- **The top bar is always visible** (it carries the window controls), so it is not hidden at small sizes — the old `updateStatusVisibility` progressive-disclosure hide was removed. The ticker text just truncates.
